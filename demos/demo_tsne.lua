...
local function show_map(method, mapped_data, input_data)

   -- draw map with original digits:
   local im_size = 2048
   local background = 0
   local background_removal = true
   map_im = manifold.draw_image_map(mapped_data, input_data, im_size, background, background_removal)
...
local function demo_tsne()
   ...
   testset.size  = N
   require 'cunn'
   testset.data  = testset.data:narrow(1, 1, N):view(5000,1,28,28):cuda()
   testset.label = testset.label:narrow(1, 1, N)
   require 'cudnn'
   local net = torch.load('model.net')
   net:evaluate()
   net:remove(#net)
   io.write('Computing the embedding...'); io.flush()
   local x = torch.CudaTensor(5000, 128)
   for i = 1, N do x[i] = net:forward(testset.data[i]) end
   x = x:double()
   print(' Done.')
   ...
   -- show_map('t-SNE', mapped_x1, testset.data:double():clone())
   ...
   show_map('Barnes-Hut t-SNE', mapped_x2, testset.data:double():clone())
end
